			    ...
			    void *data)
{
	...
	case WCD_EVENT_POST_DAPM_MICBIAS_2_ON:
		...
		break;
	case WCD_EVENT_POST_MICBIAS_2_ON:
/*** BSP : added by hmct to set  micbias2 2.8v,just config micbias_enable as true,the driver will set micbias 2.8v @{ */
#ifdef CONFIG_HIS_AUDIO_MICBIAS2_2POINT8
		mbhc->micbias_enable = true;
#endif /* CONFIG_HIS_AUDIO_MICBIAS2_2POINT8 */
/*** @} */	
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	WCD_MBHC_RSC_LOCK(mbhc);
	/* send event to sw intr handler add by hmct*/
