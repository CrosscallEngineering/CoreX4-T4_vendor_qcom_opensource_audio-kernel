...
static int wcd_check_cross_conn(struct wcd_mbhc *mbhc)
{
	...
	/// BSP: hmct deleted this check for three-pole headphone detection  @{
	#if 0 
	...
	while (!is_spl_hs)  {
		...
		if (!(hs_comp_res)) {
			pr_info("%s: Special headset detected in %d msecs\n",
					__func__, (delay * 2));/*hmct modified*/
			is_spl_hs = true;
		}
...
/***hmct changed BSP : added for headset detection to solve headset can not be detected when voice calling @{ */
#if 0	
	int cross_conn;
	int try = 0;
#endif
	 ...
	 */
    /*hmct modified, reset MBHC_FSM,before check button INT*/
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN, 0);
	wcd_enable_curr_micbias(mbhc, WCD_MBHC_EN_MB);
	...
	/*
	* hmct changed ,if selfie device is detected as MBHC_PLUG_TYPE_HEAD in function wcd_mbhc_detect_plug_type,
	...
	*/
	...
	if (!rc) {
		...
		{
			/*
			* calc impedance to check whether the device is selfie add by hmct
			*/
			WCD_MBHC_RSC_LOCK(mbhc);
	...
	/*** BSP : added for headset detection to solve headset can not be detected when voice calling @{ */
 #if 0
	do {
  ...
  #endif
	/* hmct add:
	*if plug type is headset , jump correct plug switch,to avoid noise caused by plug in headset,when camera recording;
	*also to avoid plug type was wrongly detected by headphone in the correct_plug_type process,when btn_press_intr is true.
	*/
	if(plug_type == MBHC_PLUG_TYPE_HEADSET){
		pr_info("%s: jump over the correct loop and report headset!\n",
			 __func__);
		goto report;
	}
	if ((plug_type == MBHC_PLUG_TYPE_HEADSET ||
...
exit:
  ...
  /*** hmct changed BSP : if plug type is is headphone or headset,remove elec irq to avoid err detection of selfie @{ */
  #if 0  
