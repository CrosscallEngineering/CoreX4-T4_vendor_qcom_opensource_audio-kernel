...
#if IS_ENABLED(CONFIG_HIS_AUDIO_EXT_SPK_PA)
enum {
	MSM_PATH_IDLE = -1,
	MSM_EAR_PATH,
	MSM_HP_PATH,
...
#endif /* CONFIG_HIS_AUDIO_EXT_SPK_PA */

//#ifdef CONFIG_AUDIO_FM_LNA_EN
#if IS_ENABLED(CONFIG_AUDIO_FM_LNA_EN)
static int fm_lna_en_flag = false;
static int fm_lna_en_bt_flag = false;
static int lna_en_count = 0;
#endif /*CONFIG_AUDIO_FM_LNA_EN*/
...
static const char * const msm_ext_spk_pa_mode_text[] = {
				"Off", "Mode_1", "Mode_2", "Mode_3", "Mode_4","Mode_5", "Mode_6", "Mode_7", "Mode_8","Mode_9", "Mode_10"};
static const struct soc_enum msm_ext_spk_pa_mode_enum[] = {
		SOC_ENUM_SINGLE_EXT(11, msm_ext_spk_pa_mode_text),
};
#endif /* CONFIG_HIS_AUDIO_EXT_SPK_PA */
...
//#ifdef CONFIG_AUDIO_FM_LNA_EN
#if IS_ENABLED(CONFIG_AUDIO_FM_LNA_EN)
static int msm_fm_lna_en_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: switch = %d\n", __func__, fm_lna_en_flag);
	ucontrol->value.integer.value[0] = fm_lna_en_flag;
	return 0;
}


static int fm_lna_en_gpio = -1;

void set_fm_lna_en_gpio(int gpio)
{
	fm_lna_en_gpio = gpio;
	return;
}
EXPORT_SYMBOL(set_fm_lna_en_gpio);
...
int msm8x16_fm_lna_power_switch(struct snd_soc_card *card, bool on)
{
	...
	if(!card){
		pr_err("%s: snd card not exist!\n", __func__);
		return -EINVAL;		
	}

	if (on) {
		rc = regulator_enable(card->vdd_fm_lna);
		if (rc) {
			dev_err(card->dev,
		...
		rc = regulator_disable(card->vdd_fm_lna);
		if (rc) {
			dev_err(card->dev,
...
EXPORT_SYMBOL(msm8x16_fm_lna_power_switch);

static int msm_fm_lna_en_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct sdm660_cdc_priv *priv = snd_soc_codec_get_drvdata(codec);
	struct wcd_mbhc *mbhc;
	struct snd_soc_card *snd_card = codec->component.card;
	...
	fm_lna_en_flag = ucontrol->value.integer.value[0];
	
	pr_info("%s: switch = %d\n", __func__, fm_lna_en_flag);
	if(snd_card){
		if(fm_lna_en_flag){
			msm8x16_fm_lna_power_switch(snd_card, true);
		} else {
			msm8x16_fm_lna_power_switch(snd_card, false);
		}		
	}
   ...
   if (!(mbhc->is_hs_inserted)) {
		if (fm_lna_en_flag){
			lna_en_count++;
			if(fm_lna_en_gpio>=0)
			   gpio_direction_output(fm_lna_en_gpio, 1);
			printk("%s: fm enable, lna_en_count = %d\n",__func__,lna_en_count);
		} else {
			lna_en_count--;
			printk("%s:lna_en_count = %d\n",__func__,lna_en_count);
			if (lna_en_count == 0) {
			    printk("%s:fm disable, disable lna\n",__func__);
				if(fm_lna_en_gpio>=0)
			      gpio_direction_output(fm_lna_en_gpio, 0);
			}
		...
		if (fm_lna_en_flag) {
			lna_en_count++;
		    printk("%s:lna_en_count = %d\n",__func__,lna_en_count);
			#if IS_ENABLED(CONFIG_AUDIO_FM_LNA_ONLY)
			printk("%s: fm enable, headset inserted,use lna only\n",__func__);
            if(fm_lna_en_gpio>=0)
			   gpio_direction_output(fm_lna_en_gpio, 1);
			#else		
			printk("%s: fm enable, headset inserted,disable lna\n",__func__);
            if(fm_lna_en_gpio>=0)
			   gpio_direction_output(fm_lna_en_gpio, 0);
			#endif			
		}
			...
			lna_en_count--;
			printk("%s:lna_en_count = %d\n",__func__,lna_en_count);
			printk("%s: fm_disable, headset inserted,disable lna\n",__func__);
            if(fm_lna_en_gpio>=0)
			  gpio_direction_output(fm_lna_en_gpio, 0);			
		}
...
static const char * const msm_fm_lna_en_text[] = {
				"Off", "On"};
static const struct soc_enum msm_fm_lna_en_enum[] = {
		SOC_ENUM_SINGLE_EXT(2, msm_fm_lna_en_text),
};

static int msm_fm_lna_en_bt_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: switch = %d\n", __func__, fm_lna_en_bt_flag);
	ucontrol->value.integer.value[0] = fm_lna_en_bt_flag;
	return 0;
}

static int msm_fm_lna_en_bt_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct sdm660_cdc_priv *priv = snd_soc_codec_get_drvdata(codec);
	...
	fm_lna_en_bt_flag = ucontrol->value.integer.value[0];
	
	pr_info("%s: switch = %d\n", __func__, fm_lna_en_bt_flag);
	if(snd_card){
		if(fm_lna_en_bt_flag){
			msm8x16_fm_lna_power_switch(snd_card, true);
		} else {
			msm8x16_fm_lna_power_switch(snd_card, false);
		}		
    ...
    if (!(mbhc->is_hs_inserted)) {
		...
		if (fm_lna_en_bt_flag) {
           ...
           if(fm_lna_en_gpio>=0) 
			...
			lna_en_count--;
			printk("%s:lna_en_count = %d\n",__func__,lna_en_count);
			printk("%s: fm disable, headset inserted,disable lna\n",__func__);
            if(fm_lna_en_gpio>=0)
			  gpio_direction_output(fm_lna_en_gpio, 0);			
		}
...
static const char * const msm_fm_lna_en_bt_text[] = {
				"Off", "Onbt"};
static const struct soc_enum msm_fm_lna_en_bt_enum[] = {
		SOC_ENUM_SINGLE_EXT(2, msm_fm_lna_en_bt_text),
};
...
//#ifdef CONFIG_AUDIO_FM_LNA_EN
#if IS_ENABLED(CONFIG_AUDIO_FM_LNA_EN)
		SOC_ENUM_EXT("FM LNA Enable", msm_fm_lna_en_enum[0],
				msm_fm_lna_en_get, msm_fm_lna_en_put),
		SOC_ENUM_EXT("FM LNA Enable Bt", msm_fm_lna_en_bt_enum[0],
				msm_fm_lna_en_bt_get, msm_fm_lna_en_bt_put),
#endif /*CONFIG_AUDIO_FM_LNA_EN*/
					     ...
					     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		*/
		...
		if (w->reg == MSM89XX_PMIC_ANALOG_MICB_1_EN) {
			snd_soc_update_bits(codec,
				MSM89XX_PMIC_ANALOG_MICB_1_VAL, 0xFF, 0xC0);
		}
...
/*** @} */
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (w->reg == MSM89XX_PMIC_ANALOG_MICB_1_EN) {
			snd_soc_update_bits(codec,
				MSM89XX_PMIC_ANALOG_MICB_1_VAL, 0xFF, 0x20);
		}
#endif /* CONFIG_HIS_AUDIO_MICBIAS1_2POINT8 */
/*** @} */
		break;
...
#if IS_ENABLED(CONFIG_HIS_AUDIO_EXT_SPK_PA)
static spinlock_t		aw_pa_lock;
static int ex_spk_pa_gpio = -1;

static int get_ex_spk_pa_gpio(void)
{
	return ex_spk_pa_gpio;
}

void set_ex_spk_pa_gpio(int gpio)
{
	ex_spk_pa_gpio = gpio;
	return;
}
EXPORT_SYMBOL(set_ex_spk_pa_gpio);

void ex_spk_pa_lock_init(void)
{
	spin_lock_init(&aw_pa_lock);
	return;
}
EXPORT_SYMBOL(ex_spk_pa_lock_init);

static void ex_spk_pa_power_enable(struct snd_soc_codec *codec, bool enable)
{
	int i;
	unsigned long flags;
	
	if(get_ex_spk_pa_gpio() >= 0) {
		if(enable){
			msleep(15);
			spin_lock_irqsave(&aw_pa_lock, flags);
			for(i = 0; i < msm_ext_spk_pa_mode; i++) {
				gpio_direction_output(get_ex_spk_pa_gpio(), 0);
				gpio_direction_output(get_ex_spk_pa_gpio(), 1);
			}
			spin_unlock_irqrestore(&aw_pa_lock, flags);
			dev_info(codec->dev, "%s: PA1 mode(%d) enabled ok\n", __func__, msm_ext_spk_pa_mode);
			msleep(15);
		} else {
			gpio_direction_output(get_ex_spk_pa_gpio(), 0);
			dev_info(codec->dev, "%s: PA1 disabled\n", __func__);
			msleep(15);
		}
	}
}
...
//#ifdef CONFIG_HIS_AUDIO_EXT_SPK_PA
#if IS_ENABLED(CONFIG_HIS_AUDIO_EXT_SPK_PA)
		ex_spk_pa_power_enable(codec, 1);
#endif /* CONFIG_HIS_AUDIO_EXT_SPK_PA */				
		break;
	case SND_SOC_DAPM_PRE_PMD:
		msm_anlg_cdc_dig_notifier_call(codec,
...
//#ifdef CONFIG_HIS_AUDIO_EXT_SPK_PA
#if IS_ENABLED(CONFIG_HIS_AUDIO_EXT_SPK_PA)
		ex_spk_pa_power_enable(codec, 0);
#endif /* CONFIG_HIS_AUDIO_EXT_SPK_PA */					   
		break;
	}
					     ...
					     unsigned long opcode, void *ptr)
{
	...
	case AUDIO_NOTIFIER_SERVICE_DOWN:
	   ...
	   /*hmct add to save the ADC volume before Modem reboot.*/
		adc1_vol = snd_soc_read(codec, MSM89XX_PMIC_ANALOG_TX_1_EN);
		adc2_vol = snd_soc_read(codec, MSM89XX_PMIC_ANALOG_TX_2_EN);
		adc3_vol = snd_soc_read(codec, MSM89XX_PMIC_ANALOG_TX_3_EN);
...
powerup:
		if (adsp_ready){
			...
			snd_soc_write(codec, MSM89XX_PMIC_ANALOG_TX_1_EN, adc1_vol);
			snd_soc_write(codec, MSM89XX_PMIC_ANALOG_TX_2_EN, adc2_vol);
			snd_soc_write(codec, MSM89XX_PMIC_ANALOG_TX_3_EN, adc3_vol);
			/*hmct add end*/
		}
		break;
	default:
