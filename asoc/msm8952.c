...
struct fm_lna_gpio_pinctrl_info {
	struct pinctrl *pinctrl;
	struct pinctrl_state *fm_lna_act;
	struct pinctrl_state *fm_lna_sus;
};
static struct fm_lna_gpio_pinctrl_info fm_lna_pinctrl_info;
#endif
	...
	struct snd_ctl_elem_value *ucontrol)
{
   ...
   if(!(mbhc->is_hs_inserted))
		msm_pri_mi2s_rx_ch = 1;
   else
		msm_pri_mi2s_rx_ch = ucontrol->value.integer.value[0] + 1;

	pr_debug("%s: msm_pri_mi2s_rx_ch = %d, is_hs_inserted =,%d \n", __func__, msm_pri_mi2s_rx_ch,mbhc->is_hs_inserted);
	return 1;
}
#else
static int msm_pri_mi2s_rx_ch_put(struct snd_kcontrol *kcontrol,
...
static int msm8x16_ext_spk_pa_init(void)
{
	...
	ext_spk_pa_gpio = of_get_named_gpio(spdev->dev.of_node,
		...
		if (ext_spk_pa_gpio >= 0){
			...
			set_ex_spk_pa_gpio(ext_spk_pa_gpio);
		}
...
//#ifdef CONFIG_AUDIO_FM_LNA_EN
#if IS_ENABLED(CONFIG_AUDIO_FM_LNA_EN)
static struct platform_device *lnapdev;
int fm_lna_en_gpio = -1;

/* add for fm lna en used internal antenna */
static int msm8x16_fm_lna_en_init(void)
{
	int ret = 0;
	struct pinctrl *pinctrl;
  
	fm_lna_en_gpio = of_get_named_gpio(lnapdev->dev.of_node,
		"qcom,fm-lna-en-gpio", 0);
	if (fm_lna_en_gpio >= 0) {
		if (gpio_is_valid(fm_lna_en_gpio)) {
			ret = gpio_request(fm_lna_en_gpio,"fm-lna-gpio");

			if (ret) {
			    pr_err("%s: gpio_request failed for fm_lna_en_gpio(ret %d)\n",
				    __func__, ret);
			    return -EINVAL;
			}
		} else {
			pr_err("%s: fm_lna_en_gpio is not valid\n", __func__);
			    return -EINVAL;
		}
        ...
        if (fm_lna_en_gpio >= 0){
			...
			set_fm_lna_en_gpio(fm_lna_en_gpio);
		}
     
		
		pinctrl = devm_pinctrl_get(&lnapdev->dev);
		if (IS_ERR(pinctrl)) {
			pr_err("%s: Unable to get pinctrl handle\n", __func__);
			return -EINVAL;
		}
		fm_lna_pinctrl_info.pinctrl = pinctrl;
		/* get pinctrl handle for headset switch pin*/
		fm_lna_pinctrl_info.fm_lna_sus= pinctrl_lookup_state(pinctrl,
							"fm_lna_sus");
		if (IS_ERR(fm_lna_pinctrl_info.fm_lna_sus)) {
			pr_err("%s: Unable to get pinctrl disable handle\n",
								  __func__);
			return -EINVAL;
		}

		fm_lna_pinctrl_info.fm_lna_act = pinctrl_lookup_state(pinctrl,
							"fm_lna_act");
		if (IS_ERR(fm_lna_pinctrl_info.fm_lna_act)) {
			pr_err("%s: Unable to get pinctrl active handle\n",
								 __func__);
			return -EINVAL;
		}

		ret = pinctrl_select_state(fm_lna_pinctrl_info.pinctrl,
					fm_lna_pinctrl_info.fm_lna_act);
		if (ret < 0) {
			pr_err("failed to configure the gpio\n");
			return ret;
		}		
...
#define FM_LNA_VDD_MIN_UV					2600000
#define FM_LNA_VDD_MAX_UV					3300000

static int msm8x16_fm_lna_power_init(struct snd_soc_card *card, bool on)
{
	int rc;
	...
	if (on) {
		if(!card->vdd_fm_lna) {
			card->vdd_fm_lna = regulator_get(card->dev, "vdd");
			if (IS_ERR(card->vdd_fm_lna)) {
				rc = PTR_ERR(card->vdd_fm_lna);
				dev_err(card->dev,
					"Regulator get failed vdd rc=%d\n", rc);
				goto err_vdd_get;
			}
		}
		if (regulator_count_voltages(card->vdd_fm_lna) > 0) {
			rc = regulator_set_voltage(card->vdd_fm_lna,
				FM_LNA_VDD_MIN_UV,FM_LNA_VDD_MAX_UV);
			if (rc) {
				dev_err(card->dev,
		...
		if(card->vdd_fm_lna){
			if (regulator_count_voltages(card->vdd_fm_lna) > 0)
				regulator_set_voltage(card->vdd_fm_lna, 0, FM_LNA_VDD_MAX_UV);
			regulator_put(card->vdd_fm_lna);
		}
	}
	return 0;
err_vdd_set_vtg:
...
#ifdef CONFIG_HIS_AUDIO_MICBIAS2_2POINT8
	S(v_hs_max, 1700);
#else
	S(v_hs_max, 1500);
#endif
...
//#ifdef CONFIG_AUDIO_FM_LNA_EN
	...
	printk("%s Get vdd_fm_lna\n",__func__);
	ret = msm8x16_fm_lna_power_init(card, true);
	if(ret < 0){
		pr_err("%s: msm8x16_fm_lna_power_init failed\n",
			__func__);
		msm8x16_fm_lna_power_init(card, false);
		goto err;
	}

	if (msm8x16_fm_lna_en_init()) {
		pr_err("%s: msm8x16_fm_lna_en_init failed\n",
			__func__);
	}
...
//#ifdef CONFIG_AUDIO_FM_LNA_EN
		...
		msm8x16_fm_lna_power_switch(card, false);
		msm8x16_fm_lna_power_init(card, false);
#endif /*CONFIG_AUDIO_FM_LNA_EN*/
